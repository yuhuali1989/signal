      ...
      // ─────────────────────────────────────────────────────────
      {
        ...
        schema: {
          ...
          silverTransform: [
            ...
            { op: 'CAN 聚合（车辆总线信号）', desc: 'CAN（Controller Area Network）是汽车内部通信总线，记录车辆底层状态：车速（m/s）、方向盘转角（deg）、油门/刹车踏板开度（%）、档位（P/R/N/D）、横纵向加速度、人工接管标志（is_takeover）、AEB 自动紧急制动触发标志（is_aeb）等。对每个 Scene 时间范围内的 CAN 帧做聚合：avg_speed_mps / max_brake_pct / has_takeover / has_aeb / 轨迹积分（ego_trajectory），写入 scene_index 对应字段。CAN 帧级表不保留（已聚合）。' },
    ...
    tables: [
      {
        ...
        fields: [
          ...
          { name: 'has_radar',        type: 'BOOLEAN',      nullable: false, pk: false, desc: '雷达数据是否完整' },
                { field: 'has_can',          type: 'BOOLEAN',      nullable: false, pk: false, desc: 'CAN 总线数据是否完整（车速/方向盘/油门/刹车/接管事件等车辆状态信号）' },
