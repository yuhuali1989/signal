...
describe('文件系统完整性', () => {
  ...
  test('每本书有 7 个章节文件', () => {
    ...
    const bookPrefixes = ['93f28994', 'ad-llm', 'ai-agent', 'inference', 'pytorch'];
...
describe('书籍完成度与论文覆盖度', () => {
  ...
  test('文章数量 >= 36', () => {
    const articles = getMdFiles(path.join(CONTENT_DIR, 'articles'));
    expect(articles.length).toBeGreaterThanOrEqual(53);
  });

  test('论文解读数量 >= 20', () => {
    const papers = readJSON(PAPERS_INDEX);
    expect(papers.length).toBeGreaterThanOrEqual(40);
  });
});
