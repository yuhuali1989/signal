...
//   - DataFile 行数 < 10万: BF 收益有限，可不开`,
    ...
    // ── 文件格式对比（Parquet / ORC / Avro / Arrow / JSON / CSV）──
    fileFormats: [
      {
        name: 'Parquet',
        icon: '🗂️',
        ...
        verdictColor: '#00b894',
        score: 5,
        encoding: '列式 + RLE/Dictionary/Delta 编码',
        compression: 'Snappy / Zstd / Gzip / LZ4',
        readPattern: '列裁剪 + RowGroup 统计裁剪',
        pros: [
          '列裁剪：只读需要的列，I/O 减少 80%+',
          '压缩率极高（Zstd 比 CSV 小 5~10x）',
          'RowGroup 级 min/max 统计，谓词下推',
          '生态最广：Spark/Trino/DuckDB/Pandas 全支持',
          'Iceberg/Delta/Hudi 默认数据文件格式',
        ],
        cons: [
          '不支持行级追加（写入必须整列重写）',
          '小文件场景 Footer 解析开销大',
        ...
        bestFor: 'OLAP 分析、特征工程、湖仓数据文件、ML 训练集',
        typicalSize: '原始 CSV 的 10~20%',
      },
      {
        name: 'ORC',
        icon: '🦉',
        ...
        verdictColor: '#a29bfe',
        score: 3,
        encoding: '列式 + RLE/Dictionary/Bloom Filter（内置）',
        compression: 'Zlib / Snappy / LZ4',
        readPattern: '列裁剪 + Stripe 统计裁剪 + 内置 Bloom Filter',
        pros: [
          'Hive 原生格式，Hive 生态最优',
          '内置 Bloom Filter（无需 Puffin 文件）',
          'ACID 事务支持（Hive ACID）',
          '压缩率略优于 Parquet（Zlib 模式）',
        ],
        cons: [
          '非 Hive 引擎支持较弱（Spark 支持但非最优）',
          'Iceberg/Delta 默认不用 ORC',
          '社区活跃度低于 Parquet',
          '读取性能略低于 Parquet（Spark 场景）',
        ],
        bestFor: 'Hive 数仓迁移场景、Hive ACID 表',
        typicalSize: '原始 CSV 的 8~15%',
      },
      {
        name: 'Avro',
        icon: '📦',
        color: '#ffa657',
        type: '行式存储',
        verdict: '消息队列 / Schema 注册',
        verdictColor: '#ffa657',
        score: 3,
        encoding: '行式 + Schema 内嵌（JSON Schema）',
        compression: 'Snappy / Deflate / Null',
        readPattern: '全行读取，不支持列裁剪',
        pros: [
          'Schema 内嵌，自描述，天然支持 Schema Evolution',
          'Kafka 消息序列化标准格式（配合 Schema Registry）',
          '行式写入，流式追加友好',
          'Iceberg Manifest 文件使用 Avro 格式',
        ],
        cons: [
          '不支持列裁剪，分析查询 I/O 开销大',
          '压缩率低于 Parquet/ORC',
        ...
        bestFor: 'Kafka 消息序列化、Iceberg 元数据文件、数据交换',
        typicalSize: '原始 JSON 的 50~70%',
      },
      {
        name: 'Arrow / Feather',
        icon: '🏹',
        ...
        verdictColor: '#e17055',
        score: 4,
        encoding: '列式 + 零拷贝内存布局（IPC 格式）',
        compression: 'LZ4 / Zstd（可选）',
        readPattern: '零拷贝列读取，SIMD 加速',
        pros: [
          '零拷贝：Pandas/Spark/DuckDB 共享内存，无序列化开销',
          'SIMD 向量化计算，CPU 利用率极高',
          'Parquet ↔ Arrow 转换极快（同列式布局）',
          'Ray Dataset / Pandas 2.0 默认内存格式',
        ],
        cons: [
          '磁盘存储效率低于 Parquet（无深度压缩）',
          '不适合长期持久化存储',
          '文件格式（Feather v2）生态不如 Parquet 广',
        ],
        bestFor: 'ML 训练数据加载（Ray/PyTorch）、跨进程内存共享、实时计算',
        typicalSize: '与原始数据相近（内存格式优先）',
      },
      {
        name: 'JSON / JSONL',
        icon: '📄',
        ...
        score: 2,
        encoding: '文本行式，UTF-8',
        compression: 'Gzip / Zstd（外层压缩）',
        readPattern: '全行解析，无统计信息',
        pros: [
          '人类可读，调试友好',
          'Schema 灵活，半结构化数据天然支持',
          'JSONL（每行一个 JSON）支持流式处理',
          'API 响应、日志、爬虫数据的通用格式',
        ],
        cons: [
          '存储效率极低（比 Parquet 大 5~10x）',
          '无列裁剪，无统计信息，查询慢',
          '解析 CPU 开销大',
          '不适合 PB 级数据存储',
        ],
        bestFor: '原始数据落地、API 数据交换、小规模调试',
        typicalSize: '基准（其他格式与之对比）',
      },
      {
        name: 'CSV / TSV',
        icon: '📊',
        ...
        verdictColor: '#b2bec3',
        score: 1,
        encoding: '文本行式，逗号/Tab 分隔',
        compression: 'Gzip（外层压缩）',
        readPattern: '全行解析，无统计信息',
        pros: [
          '通用性最强，Excel/数据库/BI 工具全支持',
          '人类可读，无需特殊工具',
          '数据导入导出的事实标准',
        ],
        cons: [
      ...
      // ── AI / 自动驾驶专用格式 ──
      {
        ...
        verdictColor: '#6c5ce7',
        score: 5,
        encoding: '消息容器（Channel + Schema + Message），支持任意序列化格式',
        compression: 'LZ4 / ZSTD（per-chunk）',
        readPattern: '时间戳索引随机访问，支持 Channel 过滤',
        pros: [
          '单文件封装多模态数据（相机/LiDAR/Radar/IMU/GPS）',
          '时间戳精确索引，支持任意时间范围随机读取',
          '替代 rosbag2，foxglove 生态原生支持',
          'Schema 内嵌，自描述，支持 Protobuf/JSON/ROS2 消息',
          '支持 S3 Range GET 远程随机访问，无需下载整个文件',
        ],
        cons: [
          '非列式存储，不适合批量特征提取分析',
          '生态相对新，工具链不如 Parquet 成熟',
          '大文件（>10GB）随机访问需要索引预热',
        ],
        bestFor: '自动驾驶车端原始数据采集、多传感器时序数据存储、回放调试',
        typicalSize: '50~200 GB/session（未压缩）',
      },
      {
        name: 'WebDataset',
        icon: '🌐',
        ...
        verdictColor: '#00b894',
        score: 4,
        encoding: 'tar 打包（每个样本一组文件：img.jpg + label.json + meta.txt）',
        compression: '外层 tar.gz 可选，内部文件保持原格式',
        readPattern: '顺序流式读取，支持 S3/GCS 直接流式加载',
        pros: [
          'S3 流式读取，无需预下载，适合多机多卡分布式训练',
          '样本打包为 tar，I/O 效率极高（顺序读取）',
          'PyTorch DataLoader 原生支持，与 torchvision 无缝集成',
          '支持任意模态混合（图像+文本+音频+点云）',
          'LAION-5B、CC12M 等大规模数据集标准格式',
        ],
        cons: [
          '不支持随机访问（顺序读取），无法高效 shuffle 单样本',
          '需要预先打包，增量更新成本高',
          '不适合需要频繁随机访问的场景',
        ],
        bestFor: '超大规模 ML 训练（>1M 样本）、多机多卡分布式训练、S3 流式加载',
        typicalSize: '取决于内容，通常 1~10 GB/shard',
      },
      {
        name: 'Lance',
        icon: '🏹',
        color: '#e17055',
        type: '列式 ML 存储',
        verdict: 'ML 数据集版本管理新标准',
        verdictColor: '#e17055',
        score: 4,
        encoding: '列式存储（类 Parquet）+ 向量索引（IVF/HNSW）',
        compression: 'ZSTD / LZ4',
        readPattern: '列裁剪 + 向量 ANN 搜索 + 随机访问',
        pros: [
          '原生支持向量列（embedding），内置 ANN 索引',
          '随机访问性能比 Parquet 快 100x（专为 ML 随机采样优化）',
          'LanceDB 原生格式，向量数据库与数据集统一存储',
          '支持 ACID 事务和数据版本管理（类 Delta Lake）',
          'PyTorch/HuggingFace Dataset 原生支持',
        ],
        cons: [
          '生态较新（2023 年发布），工具链不如 Parquet 成熟',
          '非 Hadoop/Spark 生态，大规模 ETL 支持有限',
          '向量索引构建耗时，不适合频繁更新场景',
        ],
        bestFor: 'ML 数据集管理（含 embedding）、向量数据库、多模态数据集随机访问',
        typicalSize: '与 Parquet 相近，向量列额外占用约 4 bytes/dim',
      },
      {
        name: 'TFRecord',
        icon: '🔷',
        color: '#4285f4',
        type: '流式序列化',
        verdict: 'TensorFlow 生态专用',
        verdictColor: '#4285f4',
        score: 2,
        encoding: 'Protocol Buffers 序列化，顺序写入',
        compression: 'GZIP（可选）',
        readPattern: '顺序流式读取，tf.data 管道',
        pros: [
          'TensorFlow/Keras 原生格式，tf.data 管道零开销',
          '支持任意数据类型（图像/文本/数值）',
          'Google Cloud TPU 训练的标准输入格式',
        ],
        cons: [
          '强绑定 TensorFlow 生态，PyTorch 需要额外转换',
          '不支持列裁剪，不支持随机访问',
          '可读性差，调试困难',
          '新项目建议用 WebDataset 或 Parquet 替代',
        ],
        bestFor: 'TensorFlow/TPU 训练管道（存量系统），新项目不推荐',
        typicalSize: '与原始数据相近（无深度压缩）',
      },
      {
        name: 'HDF5',
        icon: '🗃️',
        ...
        verdictColor: '#a29bfe',
        score: 3,
        encoding: '层次化分组（Group/Dataset），支持任意维度数组',
        compression: 'GZIP / LZF / BLOSC（per-dataset）',
        readPattern: '随机访问（h5py 内存映射），支持切片读取',
        pros: [
          '支持任意维度数组（图像序列/点云/时序数据）',
          '内存映射（mmap），大文件随机访问高效',
          '层次化结构，适合多模态数据打包（图像+标注+元数据）',
          '科学计算标准格式，NumPy/SciPy 原生支持',
        ],
        cons: [
          '单文件并发写入有锁竞争，不适合分布式写入',
          '文件损坏风险高（单文件存储所有数据）',
          '不支持列裁剪，大文件读取效率低于 Parquet',
          '云存储（S3）支持有限，需整文件下载',
        ],
        bestFor: '自动驾驶多模态训练包、科学计算数据集、单机大规模数组存储',
        typicalSize: '取决于内容，通常比 Parquet 大 20~50%',
      },
    ],

    // ── 文件格式对比矩阵（扩展版，含 AI/自动驾驶专用格式）──
    fileFormatMatrix: [
      { dim: '存储模式',     parquet: '列式',      orc: '列式',      avro: '行式',      arrow: '列式内存',  json: '行式文本', csv: '行式文本', mcap: '消息容器',   webdataset: 'tar流式',  lance: '列式+向量',  tfrecord: '流式序列化', hdf5: '层次数组' },
      { dim: '压缩率',       parquet: '⭐⭐⭐⭐⭐', orc: '⭐⭐⭐⭐⭐', avro: '⭐⭐⭐',    arrow: '⭐⭐',       json: '⭐',       csv: '⭐',       mcap: '⭐⭐⭐⭐',    webdataset: '⭐⭐⭐',    lance: '⭐⭐⭐⭐',     tfrecord: '⭐⭐',       hdf5: '⭐⭐⭐' },
      { dim: '随机访问',     parquet: '✅ RowGroup', orc: '✅ Stripe', avro: '❌',        arrow: '✅ 零拷贝',  json: '❌',       csv: '❌',       mcap: '✅ 时间戳',  webdataset: '❌ 顺序',  lance: '✅ 最优',     tfrecord: '❌',         hdf5: '✅ mmap' },
      { dim: '流式写入',     parquet: '❌',         orc: '❌',         avro: '✅',         arrow: '✅',         json: '✅',        csv: '✅',        mcap: '✅',         webdataset: '✅',        lance: '✅',           tfrecord: '✅',          hdf5: '⚠️ 有锁' },
      { dim: '多模态支持',   parquet: '⚠️ 结构化',  orc: '⚠️ 结构化',  avro: '⚠️ 结构化',  arrow: '⚠️ 结构化',  json: '✅ 灵活',  csv: '❌',        mcap: '✅ 原生',    webdataset: '✅ 打包',   lance: '✅ 含向量',   tfrecord: '✅',          hdf5: '✅ 原生' },
      { dim: '向量索引',     parquet: '❌',         orc: '❌',         avro: '❌',         arrow: '❌',         json: '❌',        csv: '❌',        mcap: '❌',         webdataset: '❌',        lance: '✅ ANN',       tfrecord: '❌',          hdf5: '❌' },
      { dim: 'ML 训练',      parquet: '✅ 主流',    orc: '⚠️',         avro: '❌',         arrow: '✅ 最优',    json: '⚠️ 小数据', csv: '❌',       mcap: '❌ 需转换',  webdataset: '✅ 大规模', lance: '✅ 原生',     tfrecord: '✅ TF专用',  hdf5: '✅ 多模态' },
      { dim: '自动驾驶',     parquet: '✅ 结构化',  orc: '❌',         avro: '⚠️ 元数据',  arrow: '✅ 实时',    json: '⚠️ 小数据', csv: '❌',       mcap: '✅ 原始采集', webdataset: '✅ 训练集', lance: '✅ 数据集',   tfrecord: '❌',          hdf5: '✅ 训练包' },
      { dim: '湖仓数据文件', parquet: '✅ 默认',    orc: '✅ Hive',    avro: '⚠️ 元数据',  arrow: '❌',         json: '❌',        csv: '❌',        mcap: '❌',         webdataset: '❌',        lance: '⚠️ 新兴',     tfrecord: '❌',          hdf5: '❌' },
      { dim: 'S3 流式读取',  parquet: '✅',         orc: '✅',         avro: '✅',         arrow: '✅',         json: '✅',        csv: '✅',        mcap: '✅ Range GET', webdataset: '✅ 原生',  lance: '✅',           tfrecord: '⚠️',         hdf5: '❌ 需下载' },
    ],

    // ── 选型决策树 ──
    selectionGuide: {
      title: '存储格式选型决策树',
      subtitle: '根据场景快速选择最合适的存储格式',
      scenarios: [
        {
          icon: '🏗️',
          title: '湖仓数据文件（Iceberg/Delta/Hudi）',
          color: '#00cec9',
          answer: 'Parquet + Zstd',
          reason: '列裁剪 + 高压缩率 + 谓词下推，是 Iceberg/Delta/Hudi 的默认格式。Zstd 比 Snappy 压缩率高 30%，解压速度相当。',
          config: "write.format.default = parquet\nwrite.parquet.compression-codec = zstd",
        },
        {
          icon: '🤖',
          title: 'ML 训练数据集（PyTorch/Ray）',
          color: '#6c5ce7',
          answer: 'Parquet（存储）→ Arrow（加载）',
          reason: 'S3 存储用 Parquet（压缩率高），DataLoader 读取时转为 Arrow 内存格式（零拷贝，SIMD 加速）。Ray Dataset 自动完成 Parquet→Arrow 转换。',
        ...
        },
        {
          ...
          title: '流式写入（Kafka → 湖仓）',
          color: '#ffa657',
          answer: 'Avro（Kafka 消息）→ Parquet（落地）',
          reason: 'Kafka 消息用 Avro + Schema Registry（Schema 演化 + 压缩）；Flink 消费后写入 Iceberg 时转为 Parquet 文件。',
          config: "# Kafka Producer\nvalue.serializer = KafkaAvroSerializer\nschema.registry.url = http://schema-registry:8081",
        },
        {
          icon: '⚡',
          title: '实时 OLAP 查询（Trino/DuckDB）',
          color: '#e17055',
          answer: 'Parquet（Iceberg 表）',
          reason: 'Trino/DuckDB 对 Parquet 的列裁剪和谓词下推支持最优，配合 Iceberg 分区裁剪，点查延迟可达秒级。',
          config: "-- Trino 查询 Iceberg 表\nSELECT col1, col2 FROM iceberg.db.table\nWHERE partition_col = '2026-04'\nAND id = 12345",
        },
        {
          icon: '🔄',
          title: '高频 Upsert / CDC 同步',
          color: '#00b894',
          answer: 'Parquet + Iceberg V3 DV',
          reason: 'Iceberg V3 Deletion Vector（位图删除）替代 V2 的 Equality Delete File，避免删除文件堆积导致的查询性能崩溃。',
          config: "-- 开启 V3 格式\nALTER TABLE t SET ('format-version'='3')\n-- 开启 DV\nALTER TABLE t SET ('write.delete.mode'='merge-on-read')",
        },
        {
          icon: '🏢',
          title: 'Hive 数仓（存量系统）',
          color: '#a29bfe',
          answer: 'ORC + Zlib',
          reason: 'Hive 对 ORC 支持最优（内置 Bloom Filter、ACID 事务），存量 Hive 数仓迁移成本最低。新建系统建议直接用 Parquet + Iceberg。',
        ...
        },
        {
          ...
          title: '本地开发 / 数据探索',
          color: '#74b9ff',
          answer: 'Parquet（DuckDB 直读）',
          reason: 'DuckDB 可直接查询本地 Parquet 文件，零配置，毫秒级响应，是本地数据探索的最佳组合。',
          config: "-- DuckDB 直读 Parquet\nSELECT * FROM read_parquet('data/*.parquet')\nWHERE ts > '2026-01-01'",
        },
        {
          icon: '📤',
          title: '数据导出 / 外部系统交换',
          color: '#b2bec3',
          answer: 'CSV / JSON（视对方系统）',
          reason: 'CSV 兼容性最强（Excel/BI 工具），JSON 适合 API 交换和半结构化数据。内部系统间交换优先用 Parquet。',
          config: "-- Spark 导出 CSV\ndf.write.csv('s3://bucket/export/', header=True)\n-- 导出 JSON\ndf.write.json('s3://bucket/export/')",
        },
        {
          icon: '🚗',
          title: '自动驾驶车端原始数据采集',
          color: '#6c5ce7',
          answer: 'MCAP',
          reason: '单文件封装 6路相机+5路LiDAR+IMU/GPS，时间戳精确索引，支持 S3 Range GET 远程随机访问，foxglove 生态原生支持，是 rosbag2 的现代替代方案。',
          config: "# Python MCAP 写入\nfrom mcap.writer import Writer\nwith open('session.mcap', 'wb') as f:\n    writer = Writer(f)\n    writer.start()\n    # 写入 camera channel\n    ch = writer.register_channel('/camera/front', 'jpeg', schema_id)\n    writer.add_message(ch, log_time=ts_ns, data=jpeg_bytes)",
        },
        {
          icon: '🌐',
          title: '超大规模 ML 分布式训练（>1M 样本）',
          color: '#00b894',
          answer: 'WebDataset (tar)',
          reason: '顺序流式读取，S3 直接流式加载，无需预下载，I/O 效率极高。LAION-5B 等大规模数据集的标准格式，PyTorch DataLoader 原生支持。',
          config: "import webdataset as wds\n# S3 流式加载\ndataset = (\n    wds.WebDataset('s3://bucket/train-{000000..001023}.tar')\n    .decode('pil')\n    .to_tuple('jpg', 'json')\n    .map(preprocess)\n)\nloader = DataLoader(dataset, batch_size=256, num_workers=8)",
        },
        {
          icon: '🏹',
          title: 'ML 数据集管理（含 embedding / 向量搜索）',
          color: '#e17055',
          answer: 'Lance',
          reason: '原生支持向量列，内置 ANN 索引，随机访问性能比 Parquet 快 100x，支持 ACID 事务和版本管理。LanceDB 原生格式，向量数据库与数据集统一存储。',
          config: "import lance\nimport pyarrow as pa\n# 写入 Lance 数据集（含 embedding 列）\nschema = pa.schema([\n    pa.field('id', pa.int64()),\n    pa.field('image', pa.binary()),\n    pa.field('embedding', pa.list_(pa.float32(), 512)),\n])\nds = lance.write_dataset(table, 'data.lance', schema=schema)\n# 向量 ANN 搜索\nresults = ds.to_table(nearest={'column': 'embedding', 'q': query_vec, 'k': 10})",
        },
      ],
    },
  },
...
// Compaction 后: DataFile 变为干净文件，DV/DeleteFile 消失，查询性能恢复基准`,
    ...
    icebergTimeline: {
      title: 'Apache Iceberg 功能时间线（2023 → 2026）',
      desc: '基于 Apache Iceberg GitHub Releases 真实发布日期整理（来源：github.com/apache/iceberg/releases），当前最新稳定版 1.10.1（2025-09-11）。',
      events: [
        {
          date: '2023-07-26',
          ...
          color: '#74b9ff',
          highlights: [
            { name: 'Bloom Filter 列索引', desc: 'Parquet Bloom Filter 正式集成写入路径，通过 write.parquet.bloom-filter-enabled.column.xxx 开启，Upsert Lookup 加速' },
            { name: 'Incremental Read API', desc: '支持只读取两个 Snapshot 之间的变更数据，CDC 场景效率大幅提升' },
            { name: 'REST Catalog 协议稳定', desc: 'REST Catalog 成为多引擎共享 Catalog 的标准方案，Polaris/Unity Catalog 均基于此协议' },
          ],
        },
        {
          date: '2023-12-27',
          ...
          color: '#74b9ff',
          highlights: [
            { name: 'V3 Spec 草案发布', desc: 'Format Spec V3 草案公开，引入 Deletion Vector、Row Lineage、Variant 类型、地理空间类型等核心特性' },
            { name: 'Puffin 统计文件格式', desc: '新增 .puffin 格式存储列级统计信息（NDV/Theta Sketch/Bloom Filter），为 V3 DV 奠定基础' },
            { name: 'PyIceberg 0.6.0', desc: 'Python 原生实现，支持 REST Catalog、谓词下推、列裁剪，无需 JVM' },
          ],
        },
        {
          date: '2024-05-16',
          ...
          color: '#a29bfe',
          highlights: [
            { name: 'V3 Spec 正式定稿', desc: 'Format Spec V3 正式发布，明确 Binary Deletion Vector、Row Lineage（_row_id）、Variant 类型、timestamp_ns、geometry/geography 类型规范' },
            { name: 'Deletion Vector 实验性支持', desc: '核心库加入 DV 读写实验性实现，Puffin 文件中 deletion-vector-v1 Blob 格式确定' },
            { name: 'Unity Catalog REST 兼容', desc: 'Unity Catalog 0.2.x 实现 Iceberg REST Catalog 协议，成为多引擎统一 Catalog 方案' },
          ],
        },
        {
          date: '2024-10-01',
          ...
          color: '#a29bfe',
          highlights: [
            { name: 'DV 写入正式支持（Spark）', desc: 'Spark 3.5+ 写入路径正式支持 V3 Deletion Vector，write.delete.mode=merge-on-read 触发 DV 路径' },
            { name: 'Row Lineage 实现', desc: '_row_id 全局唯一行 ID 分配，DataFile 新增 first-row-id，next-row-id 写入 TableMetadata' },
        ...
        },
        {
          ...
          color: '#fd79a8',
          highlights: [
            { name: 'V3 DV/Row Lineage/Variant 稳定 API', desc: 'V3 核心特性全部进入稳定 API，不再是实验性；DV 写入在 Spark 3.5+ 生产可用' },
            { name: 'PyIceberg 0.8 V3 支持', desc: 'PyIceberg 支持 V3 Variant 读写、DV 读取，Python 生态可直接操作 V3 表' },
            { name: 'Trino 460+ DV 读取 GA', desc: 'Trino 460+ 正式支持 V3 DV 读取，查询 V3 表时自动应用 Bitmap 过滤' },
          ],
        },
        {
          date: '2025-03-19',
          version: 'Iceberg 1.9.0',
          color: '#fd79a8',
          highlights: [
            { name: 'Row-id 段批量分配', desc: '解决多并发写入者 next-row-id CAS 竞争问题，Catalog 端批量分配 row-id 段，高并发写入性能提升' },
            { name: 'Flink 1.20 DV 写入支持', desc: 'Flink connector 支持 V3 DV 写入路径，流式 Upsert 不再产生 Delete File 堆积' },
            { name: 'DuckDB 1.2+ V3 读取', desc: 'DuckDB 1.2+ 支持 V3 格式读取，Variant 类型对接 JSON 生态，OLAP 分析场景覆盖更完整' },
          ],
        },
        {
          date: '2025-07-18',
          version: 'Iceberg 1.10.0',
          color: '#55efc4',
          highlights: [
            { name: 'Spark 4.0 深度集成', desc: 'Iceberg 1.10 配合 Spark 4.0 正式 GA，V3 全特性在 Spark 4.0 生产验证，MERGE INTO 性能再提升 20%+' },
            { name: 'Geometry/Geography 类型 GA', desc: '地理空间类型正式稳定，支持 WKB 编码存储，Trino/Spark 均可查询地理空间数据' },
            { name: 'REST Catalog v2 协议', desc: 'REST Catalog 协议升级 v2，支持事务性多表操作（Multi-Table Commit），解决跨表原子性问题' },
          ],
        },
        {
          date: '2025-09-11',
        ...
        {
          date: '2026-Q2（预计）',
          version: 'Iceberg 1.11.x（开发中）',
          color: '#b2bec3',
          highlights: [
            { name: 'Flink 2.1 DV 写入完整支持', desc: 'Flink 2.1（预计 2026 Q2 GA）将完整支持 V3 DV 写入路径，彻底解决 Flink 流式写入小文件爆炸问题' },
            { name: 'Iceberg Catalog Federation', desc: '多 Catalog 联邦查询，跨 Polaris/Unity Catalog/Hive Metastore 的统一视图' },
            { name: 'V3 Variant 引擎全覆盖', desc: 'Trino/Presto/DuckDB 全面支持 Variant 类型读写，半结构化数据处理能力对齐 Delta Lake' },
          ],
        },
      ],
    },
