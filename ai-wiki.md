...
### 8. 广告业务 `/ads/`
- ✅ **业务分组**下的互联网广告系统知识模块（方案 A 骨架版，后续加交互可视化）
- ✅ **4 大 Tab**：
  - 🏗️ **架构演进**：2020→2026 六代技术栈时间线（DNN → Transformer → LLM 原生）+ 经典广告漏斗（召回→粗排→精排→重排→创意）
  - 🤖 **AI 创新**：召回（双塔/图/生成式）· 排序（DIN/SIM/MMoE/Transformer）· 创意（DCO/AIGC）· 归因（MTA/Uplift/MMM/GeoLift）四大方向
  - 🧠 **大模型应用**：生成式创意（Meta Advantage+/Google PMax/字节 Seed-Ad/Amazon）· 商品用户理解 · 对话式广告（ChatGPT Ads/Perplexity）· Agent 投放员
  - 🔭 **标杆案例**：Meta Advantage+ · Google PMax · 字节 Seed-Ad（链接至 /vla/ 深度剖析）· Amazon Sponsored Brands
- ✅ 与 `/vla/` 下 Seed-Ad 深度剖析互相链接，避免重复

### 9. 金融业务 `/finance/`
- ✅ **业务分组**下的金融行业大模型 + 隐私计算知识模块（方案 A 骨架版）
- ✅ **4 大 Tab**：
  - 🧠 **大模型应用**：银行（智能客服/对公信贷/内部 Copilot/AML）· 券商投研（研报生成/路演问答/另类数据）· 保险（核保核赔/智能顾问/反欺诈）· 跨行业通用（风控/合规/RPA+LLM）
  - 🔐 **隐私计算原理**：FL 联邦学习 · HE 同态加密 · MPC 安全多方计算 · TEE 可信执行环境，每项含原理/公式/优缺点/主要厂商 + 四大技术横向对比表
  - 🏦 **金融场景应用**：多机构联合风控 · 央行数字货币 · 跨境数据合规 · 证券联合建模 · **大模型×隐私计算交叉**（TEE LLM 推理/联邦 LoRA）
  - 🔭 **标杆案例**：蚂蚁 SecretFlow · 微众 FATE · JPMorgan/Goldman · Morgan Stanley×OpenAI · Apple PCC · 建行/工行大模型
