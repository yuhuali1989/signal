...
const TIMELINE = [
  ...
  { date: '2026-04', model: 'Google Gemma 4（31B / 26B MoE）', innovation: 'Multi-Token Prediction K=4——训练范式革新：多步预测联合损失 + K 共享预测头', contribution: '🚀 训练范式创新：将传统单步交叉熵损失扩展为 K=4 多步加权联合损失（λ=1/0.5/0.25/0.125 递减），每个 token 在训练时提供 4 倍梯度信号密度，数据效率显著提升；骨干网络共享，增加 K 个轻量预测头作为额外训练目标（推理时可移除）。⚡ 推理副产品：训练好的多预测头可直接用于 Speculative Decoding（草稿→验证两步），获得 2.7× 推理加速。首次在完整系列（2B→31B）统一验证 MTP 从 671B 大模型到 2B 端侧模型的普适性。原生多模态 + 256K 上下文，Apache 2.0', tag: 'MTP+多模态', color: 'bg-green-100 text-green-700' },
  ...
  { date: '2026-05', model: 'MiniCPM5-1B（面壁智能）', innovation: '1B 参数端侧基座，AA-Index 超越所有 sub-2B 模型', contribution: '仅 1B 参数在 AA-Index 超越所有 2B 以下模型（含 Qwen3.5-2B、Gemma 4 E2B）；Base 版使用 AI 自写 Forge 框架训练；可在手机和浏览器端直接推理，证明小参数+高质量数据的路径有效', tag: '端侧小模型', color: 'bg-green-100 text-green-700' },
