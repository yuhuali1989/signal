...
- ✅ **AI Infra 技术栈** Section（DataInfraViz 组件，**12 个 Tab**）：
  - 全景总览 · K8s & 容器 · **数据湖仓**（多模态存储方案深度细化）· 数据流水线 · **计算引擎选型**（新增）· Unity Catalog · MLOps 实验 · 可观测性 · 向量 & 特征 · 图像去重 · 数据合成
  ...
  - 🏞️ **数据湖仓 Tab（自动驾驶多模态存储方案，6 子页，深度细化）**：
    - **数据链路**：车端采集（5模态 MCAP 容器）→ Landing Zone（S3，7天）→ Bronze（解码+对齐，按模态拆分）→ Silver（清洗+场景切分）→ Gold（自动标注+训练就绪），每层标注格式/分区/生命周期/日数据量
    - **模态存储规格**：5 模态（相机/LiDAR/雷达/标注/语言）各自的原始格式/存储格式/压缩策略/分区键/访问模式/训练格式/读取带宽
    - **训练集构建**：6 步流程（场景采样→多模态打包→时序窗口→版本化→预计算特征→注册 Unity Catalog）+ 输出规格（~500K 场景/~50K shards/~50TB/~15GB/s 读取带宽）
    - **IO 优化**：存储层次结构（GPU显存→NVMe缓存→Lustre→S3）+ 6 大优化策略（JuiceFS缓存/WebDataset顺序读/多进程预取/Lustre并行FS/预计算特征/DALI GPU解码）
    - **Iceberg**：6 大核心特性 + 三格式对比（Iceberg/Delta Lake/Hudi）+ 4 查询引擎（Trino/Spark SQL/DuckDB/Flink SQL）
    - **LakeFS 版本**：6 步 Git-like 工作流 + LakeFS/DVC/MLflow 三位一体版本体系
  - ⚡ **计算引擎选型 Tab**（4 子页，新增）：
...
- ✅ **闭环 Infra 链路** Section（DataLoopArch 组件，**10 层多模态闭环**，Unity Catalog 升级为独立顶层阶段）：
  - 数据采集 → 数据上传 → 数据处理 → 多模态融合 → 场景挖掘 → 数据存储 → **Unity Catalog**（独立阶段，与 MLflow 同级）→ 模型训练 → 模型部署 → 效果监控
  - 🗂️ **Unity Catalog 独立 Tab（6 子页，与 MLflow 同级别核心组件）**：
    - **三层命名空间**：5 Catalog（raw_data/processed_data/feature_store/model_registry/governance）× Schema × Table/Volume + 三种部署模式
    - **非结构化数据 Volume**：6 类 Volume（相机帧/点云/标注包/模型权重/仿真场景/视频回放）统一纳入 Catalog 命名空间，与结构化表共享血缘和权限
    - **模型管理**：Staging→Candidate→Production→Archived 四阶段生命周期 + UC vs 纯 MLflow 六维对比（存储/版本/权限/血缘/非结构化/跨团队共享）
    - **数据血缘**：列级血缘全链路（原始帧→BEV特征→训练集→模型→评测结果）+ 影响分析/根因定位/合规审计
    - **访问控制**：RBAC+ABAC 六角色权限矩阵 + 行级过滤 + 列级脱敏（GDPR）
    - **工具集成**：8 个工具（Spark/Airflow/MLflow/Feast/dbt/LakeFS/Great Expectations/Trino）
  - 🔀 **多模态融合 Tab**（4 子页）：BEV 融合流水线 · 时序对齐策略 · 跨模态标注协议 · 数据集规格
  - ⛏️ **场景挖掘 Tab**（4 子页）：场景分类体系 · 挖掘技术栈 · 主动学习闭环 · 飞轮量化指标
  - 🗄️ **数据存储 Tab**（6 子页）：分层存储 · Unity Catalog 命名空间 · 数据血缘 · 访问控制 · 数据质量 · 特征仓库
  - 🏗️ **架构全景图**：升级为 10 层全景（新增 Unity Catalog 独立层，位于存储层和训练层之间）
