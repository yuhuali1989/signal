...
- ✅ **专业 SVG 架构图升级**（2026-04-29）：新增 `ArchDiagram.js` 组件，**Sebastian Raschka 风格**：
  - 白底 + 灰色嵌套圆角框 + 粉色 Attention Block + ⊕ 残差符号 + 品红色参数标注
  - `TransformerBlockSVG`：Decoder-Only 架构图（自下而上流向，右侧 FFN 展开细节，左侧 RoPE 外挂）
  - `MoEArchSVG`：MoE 架构图（Router + Experts 可视化 + MTP 虚线标注）
