...
- ✅ **AI Infra 技术栈** Section（DataInfraViz 组件，**12 个 Tab**）：
  ...
  - 🏞️ **数据湖仓 Tab（自动驾驶多模态存储方案，10 子页，深度细化）**：
    - **存储格式全景**（默认展示）：5阶段格式决策地图，每阶段含格式裁决/选型理由/Schema表格/访问方式代码/**存储介质详细介绍**（规格/选型理由/生命周期/成本参考/备选方案对比）；阶段1和阶段5新增 **MCAP vs WebDataset 不能统一的深度原因**（访问模式冲突/标注时序/预处理不可逆）
    - **车端采集客户端**：软件栈/MCAP文件结构（14个Channel）/时间戳同步/Session切割/增量读取/多模态拼接规则
    - **数据链路**：车端采集→Landing→Bronze→Silver→Gold 五层链路
    - **Schema 设计**：数据粒度层次（Trip/Session/Scene/Frame）+ **极简双表设计**：
      - `session_index`（Bronze层唯一表，1行=1个MCAP文件）
      - `scene_index`（Silver/Gold层唯一表，1行=1个Scene，含CAN/雷达/标注聚合字段）
      - **所有帧级表均已删除**（lidar_frames/radar_frames/can_bus/bbox_3d/seg_masks/language_qa）
      - 训练时：一张表，零JOIN，直接读 `webdataset_shard` 字段
