...
>
> **当前数据快照（2026-04-24）**：
> - 书籍 **13 本**（含 4 本开源项目源码解析系列）· 文章 **82 篇** · 论文解读 **64 篇**（index 69 条） · 模型 **72 个** · 声浪 **110 条** · 进化日志 **194 条**
> - 导航条目共 **16 个**，分四大分组
...
- 🌊 **数据流水线 Tab（2 子页）**：
    ...
    - **Airflow 源码解析**（7 子页，详细介绍 Airflow 核心源码）：
      - **架构总览**：6 大核心组件（Scheduler/KubernetesExecutor/DAG Processor/Metadata DB/Web Server/Triggerer）各自的源码文件路径、核心类/方法、执行流程
      - **Scheduler**：`_run_scheduler_loop()` 主调度循环源码 + TaskInstance 状态机（none→scheduled→queued→running→success/failed/deferred）
