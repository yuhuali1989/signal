...
export const LOOP_OVERVIEW = {
  ...
  stages: [
    ...
    { id: 'store',       label: '数据存储',   icon: '🗄️', color: '#ffa657', desc: '数据湖 + 分层存储 + 特征仓库' },
    { id: 'unityCatalog', label: 'Unity Catalog', icon: '🗂️', color: '#e84393', desc: '统一元数据 · 模型管理 · 湖表 · 非结构化数据' },
...
export const UNITY_CATALOG_LAYER = {
  ...
  // Volume：多模态非结构化数据管理（新增）
  volumes: {
    title: 'Volume — 多模态非结构化数据统一管理',
    desc: 'Unity Catalog Volume 类型，将图像/点云/视频/模型权重等非结构化文件纳入统一治理',
    types: [
      {
        name: '相机帧 Volume',
        path: 'raw_data.camera.frames_volume',
        format: 'JPEG / H.265 / MCAP',
        size: '~180 TB/月',
        access: 'Spark + Python SDK + REST API',
        color: '#6c5ce7',
        icon: '📷',
        desc: '环视相机原始帧，支持随机访问和流式读取',
      },
      {
        name: '点云 Volume',
        path: 'raw_data.lidar.pointcloud_volume',
        format: 'MCAP / PCD / Draco',
        size: '~30 TB/月',
        access: 'Open3D + PyTorch DataLoader',
        color: '#3fb950',
        icon: '🟢',
        desc: '128线 LiDAR 点云，Draco 压缩存储',
      },
      {
        name: '标注数据 Volume',
        path: 'processed_data.annotations.label_volume',
        format: 'nuScenes JSON + WebDataset tar',
        size: '~5 TB/月',
        access: 'WebDataset + Petastorm',
        color: '#fd79a8',
        icon: '🏷️',
        desc: '多模态联合标注包，训练直接消费',
      },
      {
        name: '模型权重 Volume',
        path: 'model_registry.models.weights_volume',
        format: 'PyTorch .pt / ONNX / TensorRT .engine',
        size: '~2 TB/月',
        access: 'MLflow Artifacts + REST API',
        color: '#3fb950',
        icon: '🧠',
        desc: '与 MLflow 实验绑定，版本化管理模型权重',
      },
      {
        name: '仿真场景 Volume',
        path: 'model_registry.evaluations.sim_volume',
        format: 'CARLA .xosc / OpenSCENARIO',
        size: '~500 GB/月',
        access: 'CARLA Python API',
        color: '#79c0ff',
        icon: '🎮',
        desc: '10万+仿真场景文件，与评测结果表关联',
      },
      {
        name: '视频回放 Volume',
        path: 'governance.audit.replay_volume',
        format: 'MP4 / ROS2 bag',
        size: '~10 TB/月',
        access: 'ffmpeg + ROS2 CLI',
        color: '#ffa657',
        icon: '🎬',
        desc: '接管事件/碰撞事件完整回放，合规留存',
      },
    ],
    advantages: [
      { point: '统一命名空间', desc: '结构化表（Iceberg）与非结构化文件（Volume）共享同一 Catalog 命名空间，SQL 可直接引用', icon: '🔗' },
      { point: '血缘贯通', desc: '从原始 Volume 文件到训练集 Table 到模型权重 Volume，全链路血缘自动追踪', icon: '🔬' },
      { point: '统一权限', desc: '同一套 RBAC 规则同时控制表数据和文件访问，无需维护两套权限系统', icon: '🔐' },
      { point: '版本管理', desc: 'Volume 支持快照和版本标签，与 LakeFS 数据版本对齐', icon: '🏷️' },
    ],
  },

  // 模型管理（强化）
  modelManagement: {
    title: '模型全生命周期管理（与 MLflow 深度集成）',
    desc: 'Unity Catalog model_registry catalog 作为 MLflow 的统一后端，实现模型版本、血缘、权限的一体化管理',
    lifecycle: [
      { stage: 'Staging',    desc: '实验阶段，MLflow run 自动注册到 experiments schema', color: '#ffa657', icon: '🧪' },
      { stage: 'Candidate',  desc: '候选模型，通过 NAVSIM 评测后晋升，写入 models schema', color: '#00cec9', icon: '🎯' },
      { stage: 'Production', desc: '生产模型，OTA 部署后标记，血缘追踪至车端版本', color: '#3fb950', icon: '🚀' },
      { stage: 'Archived',   desc: '归档模型，权重文件移至冷存储 Volume，元数据永久保留', color: '#79c0ff', icon: '📦' },
    ],
    vsMLflow: [
      { aspect: '模型存储',   mlflow: 'MLflow Artifact Store（S3/本地）', uc: 'model_registry.models.weights_volume（统一命名）' },
      { aspect: '版本管理',   mlflow: 'MLflow Model Version（独立）', uc: 'UC 版本 + LakeFS commit 双重版本' },
      { aspect: '权限控制',   mlflow: '无细粒度权限', uc: 'RBAC 到 Schema/Table/Volume 级别' },
      { aspect: '血缘追踪',   mlflow: '仅实验内部', uc: '跨系统列级血缘（数据→特征→模型→部署）' },
      { aspect: '非结构化',   mlflow: '仅支持 Artifact', uc: 'Volume 统一管理权重/仿真/回放文件' },
