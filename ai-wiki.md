...
- ✅ **专业 SVG 架构图升级**（2026-04-29）：新增 `ArchDiagram.js` 组件，包含 3 个 SVG 架构图：
  - `TransformerBlockSVG`：Decoder-Only Transformer 标准架构图（Apple 风格配色，替代 ASCII 字符画）
  - `MoEArchSVG`：DeepSeek V3 MoE 架构图（MLA + Router + 256 细粒度专家 + MTP）
  - `EvolutionPathSVG`：通用演进路线图组件（5 条路径 SVG 总览，标注代表模型）
  - ModelHub 架构图库展开详情自动选择 MoE/标准 SVG 图；ASCII 字符画保留为折叠备选
