...
- ✅ **闭环 Infra 链路** Section（DataLoopArch 组件，**9 层多模态闭环**，已深度细化）：
  - 数据采集 → 数据上传 → 数据处理 → **多模态融合**（新增）→ **场景挖掘**（新增）→ 数据存储 → 模型训练 → 模型部署 → 效果监控
  - 🔀 **多模态融合 Tab**（4 子页）：BEV 融合流水线（LSS/VoxelNet/RadarNet/BEVFusion/语言注入/BEVFormer 6 步）· 时序对齐策略（PTP 硬件同步 <1μs / FPGA 触发 / 软件插值 / 跨帧时序建模）· 跨模态标注协议（相机/LiDAR/雷达/语言/轨迹 五模态联合标注 + 四级质检体系）· 多模态数据集规格（格式/采样率/时序窗口/模态覆盖率）
  - ⛏️ **场景挖掘 Tab**（4 子页）：场景分类体系（长尾/困难/安全关键/覆盖缺口/多样性 五大类）· 挖掘技术栈（CLIP嵌入→FAISS索引→孤立森林→HDBSCAN聚类→主动学习→数据平衡 6 步）· 主动学习闭环（标注效率提升 3-5×）· 数据飞轮量化指标（长尾覆盖率/困难样本比例/标注自动化率/多样性指数）
  - 🏗️ **架构全景图**：升级为 9 层多模态闭环全景（含多模态融合层和场景挖掘层详细组件），存储层新增 Unity Catalog 元数据组件标注
  - 🗂️ **数据存储 Tab（Unity Catalog 深度集成，6 子页）**：
    - **分层存储**：热/温/冷三级 + 存储规模统计
    - **Unity Catalog**：5 个 Catalog（raw_data / processed_data / feature_store / model_registry / governance）× 三层命名空间（Catalog→Schema→Table）+ 8 个集成工具（Spark/Airflow/MLflow/Feast/dbt/LakeFS/Great Expectations/Trino）+ 三种部署模式（Databricks 托管/开源自托管 K8s/混合）
    - **数据血缘**：列级血缘追踪全链路（原始帧→BEV特征→训练集→模型→评测结果）+ 影响分析/根因定位/合规审计 4 大能力
    - **访问控制**：RBAC+ABAC 六角色权限矩阵 + 行级过滤（按车辆ID/城市/时间）+ 列级脱敏（PII 自动处理，GDPR 合规）
    - **数据质量**：6 条内置 DQ 规则（时间戳连续性/IoU阈值/NULL率/特征漂移/模型指标/PII合规）+ 自动修复 + Grafana 告警
    - **特征仓库**：Feast 特征定义 + Unity Catalog feature_store catalog 映射 + 血缘追踪至原始传感器
