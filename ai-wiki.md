...
- 排查发现：
  - 大量 link 指向**官网首页 / 新闻列表 / 博客首页**（如 `anthropic.com/news`、`databricks.com/blog`、`salesforce.com/news/`、`qianxin.com/`、`36kr.com/newsflashes`），违反"本条动态原文完整 URL"的规定
  - 根本原因：**动态本身是 AI 编造的**（使用今天的日期 `2026-04-21`，事件未真实发生，如 "Anthropic $75 亿 D 轮 $1500 亿"、"Agentforce 2.0 GA"、"Qwen3-Max"、"Gartner 2026 AI 软件 $2510 亿报告" 等），于是 link 只能拿官网首页兜底
- 处置：删除 id 601-2510 共约 900 行（全部不可信动态），保留年度汇总（id 501+，无 link 静态卡）+ 新增 id 1001-1020 共 20 条**2024-2025 年真实可查事件**（每条附带可稳定命中的官方 URL）
- 教训（未来自动化任务必须遵守）：
  - ❌ **永远不允许写未来日期**。`date` 必须 ≤ 当日，且是该事件在原文里的真实发布日
  - ❌ **没有真实 URL = 不要这条**。link 字段缺失时必须整条删除（而不是用官网首页兜底）
  - ❌ **"每日 10 条"不是硬指标**。真实世界并非每天都有 10 条值得写的软件行业事件
  - ✅ 遇到你熟悉但拿不准 URL 的事件，宁可标 `source` 为"年度汇总/季度汇总"，**不带 link**，让 UI 降级为静态卡
