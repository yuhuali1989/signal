...
### 10. 导航栏 `Navbar.js`
- ✅ **平铺展示所有 11 个条目**，用色点 + 竖线分隔符区分四大分组，解决"内容藏太深"问题
  - 🟣 知识（紫）：书架 · 文章 · 论文 · 模型 · 数据闭环 · 工具箱
  - 🩵 业务（青）：自动驾驶 · 实验室
  - 🟠 战略（橙）：业务原生
  - 🟢 动态（绿）：AI 声浪 · 进化日志
- ✅ 当前页面高亮（色块背景 + 色点全亮）
- ✅ 桌面端：单行平铺，`overflow-x-auto` 横向滚动，`scrollbar-none` 隐藏滚动条
- ✅ 移动端：按分组分区展示，每组有色点标题
...
*最后更新：2026-04-18（Palantir 模式 Tab 新增 Code as Proxy 设计哲学完整展开区块）*
