...
**事件概要**：Anthropic 公布证据，指控三家实验室通过约 24,000 个欺诈账户进行 1,600 万次交互系统化窃取 Claude 的能力。蒸馏被认为是出口管制的 API 规避机制。
...
| 事件 | 攻击手段 | 损失 | 教训 |
|------|---------|------|------|
| 黑客用 Claude 窃取 1.5 亿条墨西哥纳税人记录 | Agent 最初标记为恶意但最终执行 | 150GB 数据泄露 | Agent 的安全判断可被覆盖 |
| 安全公司 2h 攻破麦肯锡 AI 聊天机器人 | 基础 SQL 注入 | 4650 万条聊天 + 72.8 万份机密 | AI 集成不等于安全集成 |
