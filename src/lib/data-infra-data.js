...
with tarfile.open(row.webdataset_shard) as tar:
  ...
  edgeClient: {
    ...
    rawIcebergRow: {
      ...
      // 关键结论
      keyInsights: [
        { insight: '相机/LiDAR Iceberg 表：file_path（调试）+ shard_path + shard_frame_offset（训练）双路径', detail: 'Bronze 层写入 file_path（单帧 JPEG/PCD）；Gold 层打包 WebDataset 后回填 shard_path 和 shard_frame_offset。训练走 shard，调试走 file_path。' },
        { insight: 'scene_id 在 Bronze 层写入时为 NULL，Silver 层场景切分后回填', detail: 'Iceberg 支持行级 UPDATE，Bronze 层先写帧，Silver 层场景切分完成后 UPDATE frames_v2 SET scene_id = xxx WHERE session_id = xxx AND timestamp_us BETWEEN ...' },
        { insight: 'video_sessions 表：行粒度 = Session（15min）', detail: '视频文件太大，不按帧存，而是整个 Session 一行，通过 session_id 与帧表关联' },
        { insight: 'scene_index 是训练采样唯一入口，不直接 JOIN 帧级表', detail: 'Silver 层切分后产生，含 dedup_hash/clip_embedding/annotation_status 字段。训练时先 SQL 过滤 scene_id，通过 webdataset_shard 字段交给 WebDataset 读取，不走帧级 JOIN' },
    ...
    tables: [
      {
        ...
        icon: '📷',
        color: '#6c5ce7',
        desc: '相机帧元数据表（Bronze 层：file_path 指向单帧 JPEG；Gold 层打包后新增 shard_path + shard_frame_offset 用于训练直读）',
        partitionBy: 'days(event_time), camera_id',
        sortBy: 'scene_id, timestamp_us',
        fields: [
          { name: 'frame_id',             type: 'STRING',    nullable: false, pk: true,  desc: '帧唯一标识，格式 {scene_id}_{timestamp_us}' },
          { name: 'scene_id',             type: 'STRING',    nullable: true,  pk: false, desc: '关联场景（Silver 层场景切分后回填，Bronze 层写入时为 NULL）' },
          { name: 'session_id',           type: 'STRING',    nullable: false, pk: false, desc: '所属 Session，关联 video_sessions 表，Bronze 层写入时即可填充' },
          ...
          { name: 'event_time',           type: 'TIMESTAMP', nullable: false, pk: false, desc: '分区字段（Iceberg 隐式分区，由 timestamp_us 转换）' },
          { name: 'file_path',            type: 'STRING',    nullable: false, pk: false, desc: 'Bronze 层单帧 JPEG 路径：s3://raw/camera/{vehicle_id}/{date}/{session_id}/{frame_id}.jpg（~150KB）' },
          { name: 'shard_path',           type: 'STRING',    nullable: true,  pk: false, desc: 'Gold 层 WebDataset shard 路径（打包后回填）：s3://gold/shards/shard_001234.tar' },
          { name: 'shard_frame_offset',   type: 'LONG',      nullable: true,  pk: false, desc: 'Gold 层：帧在 shard tar 包内的字节偏移，配合 shard_path 实现调试时随机访问单帧' },
          ...
          { name: 'intrinsic_json',       type: 'STRING',    nullable: true,  pk: false, desc: '相机内参 JSON（fx/fy/cx/cy/畸变系数），每帧记录防止标定变化' },
          { name: 'extrinsic_json',       type: 'STRING',    nullable: true,  pk: false, desc: '外参 JSON（车体坐标系变换矩阵 4×4）' },
          { name: 'is_blurry',            type: 'BOOLEAN',   nullable: false, pk: false, desc: '模糊帧标记（Laplacian 方差 < 100），质量过滤用' },
          { name: 'quality_score',        type: 'FLOAT',     nullable: true,  pk: false, desc: '图像质量分（0-1）' },
          { name: 'camera_aligned',       type: 'BOOLEAN',   nullable: true,  pk: false, desc: '是否与 LiDAR 帧成功对齐（±5ms 内找到对应 LiDAR 帧），Silver 层处理后回填' },
        ],
        volumeRef: 'Bronze: raw_data.camera.frames_volume（单帧 JPEG）| Gold: WebDataset shard（通过 shard_path 字段）',
        note: '⚠️ file_path 指向单帧 JPEG（Bronze 层调试用）；训练时应使用 shard_path + shard_frame_offset 读取 WebDataset shard，避免 2.16 亿小文件 IO 问题',
      ...
      },
      {
        ...
        icon: '🟢',
        color: '#3fb950',
        desc: 'LiDAR 点云元数据表（Bronze 层：file_path 指向单帧 PCD；Gold 层打包后新增 shard_path + shard_frame_offset）',
        partitionBy: 'days(event_time)',
        sortBy: 'scene_id, timestamp_us',
        fields: [
          { name: 'frame_id',           type: 'STRING',    nullable: false, pk: true,  desc: '帧唯一标识，格式 {scene_id}_{timestamp_us}' },
          { name: 'scene_id',           type: 'STRING',    nullable: true,  pk: false, desc: '关联场景（Silver 层场景切分后回填）' },
          { name: 'session_id',         type: 'STRING',    nullable: false, pk: false, desc: '所属 Session，Bronze 层写入时填充' },
          ...
          { name: 'file_path',          type: 'STRING',    nullable: false, pk: false, desc: 'Bronze 层单帧 PCD 路径：s3://raw/lidar/{vehicle_id}/{date}/{session_id}/{frame_id}.pcd.draco（~500KB）' },
          { name: 'shard_path',         type: 'STRING',    nullable: true,  pk: false, desc: 'Gold 层 WebDataset shard 路径（打包后回填）' },
          { name: 'shard_frame_offset', type: 'LONG',      nullable: true,  pk: false, desc: 'Gold 层：帧在 shard tar 包内的字节偏移' },
          ...
          { name: 'ego_pose_json',      type: 'STRING',    nullable: true,  pk: false, desc: '采集时刻车体位姿 JSON（4×4 变换矩阵，坐标系转换用）' },
          { name: 'bbox_3d_count',      type: 'INT',       nullable: true,  pk: false, desc: '自动标注 3D 框数量（Gold 层标注完成后回填）' },
        ],
        volumeRef: 'Bronze: raw_data.lidar.pointcloud_volume（单帧 .pcd.draco）| Gold: WebDataset shard（通过 shard_path 字段）',
        note: '⚠️ 与 frames_v2 相同策略：file_path 用于调试，训练时用 shard_path + shard_frame_offset',
